function [PostOA, PostUN] = postTraining(sID, fromMain, expparam)
...
sIDDir = [dataDir filesep num2str(sID) filesep];
if ~exist(sIDDir, 'dir')
    fprintf('Folder %s doesn not exist\n', sIDDir);
else
    filename = [sIDDir num2str(sID) 'parameters.mat'];
    load(filename)
    % Get date and time
    time = clock;
    ...
    for s = 1 : 8 %8 how many sequences
        ...
        DrawFormattedText(win, ' * ', Squares(1,3), Squares(2,3), grey);
        Screen('Flip', win);
        WaitSecs(1);
        % sequence
        %Screen('FrameRect', win, black, border );
        for i = 1:5
            DrawFormattedText(win, [' ' num2str(Sequences(s,i))], Squares(1,i), Squares(2,i)-7, grey);
        end
        Screen('Flip', win);
        WaitSecs(2.7);
        ...
        for r = 1:5 % 5 repetitions
            ...
            % 5 black asterix
            for i = 1:5
                DrawFormattedText(win, ' * ', Squares(1,i), Squares(2,i)-40, grey);
            end
            %Screen('FrameRect', win, black, border );
            ...
            for k = 1:5 % 5 keys
                ...
                OUTPUT = [OUTPUT; sID s k tpSinceSeqStart e sNR(s)];
    ...
    PostOA = [sID nanmean(PostOAavgs(:,2:6))];
    PostUN = [sID nanmean(PostUNavgs(:,2:6))];
    
    filename = [sIDDir num2str(sID) '_postTraining_' datetime '.mat'];
    save(filename);
    if ~fromMain
        % get back to Matlab
        clear Screen;
